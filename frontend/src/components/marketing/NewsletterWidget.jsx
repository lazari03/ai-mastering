  ...
  if (status === "done") {
    return (
      <div className={size === "lg" ? "rounded-2xl border border-brass/30 bg-brass/[0.06] p-6" : "rounded-xl border border-brass/25 bg-brass/[0.06] p-4"}>
        <p className="m-0 text-sm font-semibold text-brass">You&apos;re on the list.</p>
        {discountCode ? (
          <>
            <p className="mt-1.5 text-xs text-zinc-300">Here&apos;s your 10% off code — use it at checkout, any time.</p>
          ...
          <p className="mt-1.5 text-xs text-zinc-400">Your discount code is on its way.</p>
        )}
      </div>
    );
  }
