...
export default function AdminSessionDetailPage() {
  ...
  useEffect(() => {
    ...
    getAdminAnalytics(`/sessions/${params.sessionId}`)
      ...
      .catch((err) => !cancelled && setError(err?.message || "Failed to load session."));
    return () => {
      cancelled = true;
    };
  }, [params.sessionId]);
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-white/10 bg-black/20 p-4">
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-400">
          <span>{session.authenticated ? "Registered" : "Anonymous"} visitor</span>
          <span>{session.utmSource || session.referrerDomain || "direct"}</span>
          <span>{session.deviceCategory}</span>
          {session.country ? <span>{session.country}</span> : null}
          <span>{session.isNewVisitor ? "New" : "Returning"}</span>
        </div>
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div>
            <p className="m-0 text-[10px] uppercase tracking-[0.1em] text-zinc-500">Total Session</p>
            <p className="mt-1 text-lg font-bold text-white">{formatDuration(totalMs)}</p>
          </div>
          <div>
            <p className="m-0 text-[10px] uppercase tracking-[0.1em] text-zinc-500">Active</p>
            <p className="mt-1 text-lg font-bold text-brass">{formatDuration(session.activeMs || 0)}</p>
          </div>
          <div>
            <p className="m-0 text-[10px] uppercase tracking-[0.1em] text-zinc-500">Pages</p>
            <p className="mt-1 text-lg font-bold text-white">{session.pageViewCount || 0}</p>
          </div>
          <div>
            <p className="m-0 text-[10px] uppercase tracking-[0.1em] text-zinc-500">Outcome</p>
            <p className="mt-1 text-sm font-semibold text-white">{session.hasPaid ? "Paid" : session.hasStartedCheckout ? "Checkout, no payment" : session.hasMastered ? "Mastered" : session.hasUploaded ? "Uploaded" : "Browsed only"}</p>
          </div>
        </div>
      </div>

      <div className="rounded-xl border border-white/10 bg-black/20 p-4">
        <p className="m-0 mb-3 text-[11px] font-bold uppercase tracking-[0.1em] text-zinc-500">Journey</p>
            ...
            .map((event) => {
              ...
              return (
                <li key={event.id} className="relative text-xs">
                  ...
                  {event.source === "backend" ? <span className="ml-1.5 rounded-full border border-white/15 px-1.5 py-0.5 text-[9px] uppercase text-zinc-500">server</span> : null}
                </li>
              );
            })}
          {events.length === 0 ? <li className="text-zinc-500">No events recorded for this session.</li> : null}
        </ol>
      </div>
    </div>
  );
